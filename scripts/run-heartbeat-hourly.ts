...
async function checkInvariants(): Promise<InvariantResult[]> {
  ...
  // 2. Daily memory freshness
  const dailyPath = path.join(repoRoot, "memory/daily", `${today}.md`);
  const dailyExists = fsSync.existsSync(dailyPath);
  if (dailyExists) {
    const stat = fsSync.statSync(dailyPath);
    ...
    if (hoursAgo >= 4) {
      ...
      fsSync.appendFileSync(
        dailyPath,
        `\n- ${stamp} Auto-refresh from hourly heartbeat to keep daily memory fresh.\n`,
      );
      results.push({ name: "Daily memory fresh", ok: true, detail: "auto-refreshed just now" });
    } else {
      results.push({ name: "Daily memory fresh", ok: true, detail: `${hoursAgo.toFixed(1)}h old` });
    }
  } else {
    const dir = path.join(repoRoot, "memory/daily");
    if (!fsSync.existsSync(dir)) {
      fsSync.mkdirSync(dir, { recursive: true });
    }
    fsSync.writeFileSync(
      dailyPath,
      `# Daily Log — ${today}\n\n_Auto-generated by heartbeat (no evening report yet)_\n`,
    );
    results.push({
      name: "Daily memory fresh",
      ok: true,
      detail: `${today}.md missing — created stub`,
    });
  }

  // 3. Session handoff freshness
  ...
  if (fsSync.existsSync(handoffPath)) {
    ...
    const hoursAgo = (now - stat.mtimeMs) / 3600000;
    results.push({
      name: "Session handoff fresh",
      ok: hoursAgo < 24,
      detail: `${hoursAgo.toFixed(1)}h old`,
    });
  } else {
    results.push({ name: "Session handoff fresh", ok: false, detail: "Missing" });
  }

  // 4. Event log has entries from today
  ...
  if (fsSync.existsSync(eventLogPath)) {
    ...
    const hasToday = lastLine.includes(today);
    results.push({
      name: "Event log active",
      ok: hasToday,
      detail: hasToday ? "entries today" : "no entries today",
    });
  } else {
    results.push({ name: "Event log active", ok: false, detail: "event-log.jsonl missing" });
  }

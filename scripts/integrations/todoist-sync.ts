...
function requireTodoistToken(): string | null {
...
function fetchTasks(token: string): TodoistTask[] {
  try {
...
function fetchCompletedTasks(token: string): TodoistTask[] {
  try {
...
async function main() {
  ...
  const completed = fetchCompletedTasks(token);
